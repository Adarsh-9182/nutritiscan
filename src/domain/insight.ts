...
// the home screen renders gracefully with no insight at all. A
// product that must always have something to say ends up saying
// "Your weight is 71.2 kg" forever, and people stop reading the
// slot.
// ============================================================

import { JULY_PANEL, markerById, delta } from "./labs";
import { IRON_3DAY_AVG, IRON_TARGET, SLEEP, hoursLabel } from "./persona";
...
export type Insight = {
  id: string;
  /** Two sentences maximum — this is a glance, not a read. */
  text: string;
  evidence: Evidence[];
  /** The single next step. Always exactly one. */
  action: { label: string; href: string };
  /** Higher wins when several qualify. */
  weight: number;
};
 ...
 *
 * Each candidate is a function of live data rather than a string:
 * if ferritin recovers, the first insight stops qualifying and
...
export function candidateInsights(): Insight[] {
  ...
  if (ferritin && IRON_3DAY_AVG < IRON_TARGET) {
    out.push({
      ...
      evidence: [
        { label: "Your labs · 12 Jul", source: "labs", href: "/labs/ferritin" },
  ...
  if (ldl && ldlMove && ldlMove.direction === "up") {
    out.push({
      ...
      evidence: [{ label: "Your labs · 12 Jul", source: "labs", href: "/labs/ldl" }],
  ...
  if (sleepNow > 0 && sleepNow < 7) {
    out.push({
      id: "sleep-short",
      text: `You're averaging ${hoursLabel(sleepNow)}. Your fatigue days cluster after the short nights — it's worth a week of watching.`,
 ...
 *
 * It was tempting to make this clever ("Big day ahead, Dev!"). A
 * health product that performs enthusiasm at 7am reads as a brand
 * talking at you. Plain is warmer.
 */
...
/** Says how much is worth the user's attention, and nothing else. */
